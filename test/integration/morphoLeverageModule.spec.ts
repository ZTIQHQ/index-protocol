...
const contractAddresses = {
  ...
  wstethDaiPool: "0x60594a405d53811d3bc4766596efd80fd545a270",
...
describe("MorphoLeverageModule integration", () => {
  ...
  let wstethDaiPool: UniswapV3Pool;
...
//         [tokenAddresses.wsteth, tokenAddresses.dai],
//         [ether(1), ether(100)],
...
//       subjectCollateralAssets = [tokenAddresses.wsteth, tokenAddresses.dai];
//       subjectBorrowAssets = [tokenAddresses.dai, tokenAddresses.wsteth];
//       subjectCaller = owner;
//     };
...
//         const enabledAssets = await morphoLeverageModule.getEnabledAssets(setToken.address);
...
//         );
//         const isDaiCollateral = await morphoLeverageModule.collateralAssetEnabled(
//           setToken.address,
//           tokenAddresses.dai,
//         );
//         const isDaiBorrow = await morphoLeverageModule.borrowAssetEnabled(
//           setToken.address,
//           tokenAddresses.dai,
//         );
//         const iswstethBorrow = await morphoLeverageModule.borrowAssetEnabled(
//           setToken.address,
//           tokenAddresses.wsteth,
//         );
...
//         expect(iswstethCollateral).to.be.true;
//         expect(isDaiCollateral).to.be.true;
//         expect(isDaiBorrow).to.be.true;
//         expect(iswstethBorrow).to.be.true;
//       });
...
//           await morphoLeverageModule.initialize(
//             setToken.address,
//             [wsteth.address, dai.address],
//             [dai.address, wsteth.address],
//           );
//         }
//         // Mint aTokens
//         await wsteth.approve(aaveLendingPool.address, ether(1000));
...
//         subjectTradeAdapterName = "UNISWAPV3";
//         subjectTradeData = await uniswapV3ExchangeAdapterV2.generateDataParam(
//           [dai.address, wsteth.address], // Swap path
//           [500], // Fees
//           true,
//         );
//         subjectCaller = owner;
...
//           const oldSourceTokenBalance = await dai.balanceOf(wstethDaiPool.address);
//           await subject();
...
//           const newSourceTokenBalance = await dai.balanceOf(wstethDaiPool.address);
...
//           const oldDestinationTokenBalance = await wsteth.balanceOf(wstethDaiPool.address);
//           await subject();
...
//           );
//           const newDestinationTokenBalance = await wsteth.balanceOf(wstethDaiPool.address);
...
//           });
//           it("should transfer the correct components to the exchange", async () => {
//             // const oldSourceTokenBalance = await dai.balanceOf(oneInchExchangeMockTowsteth.address);
//             await subject();
//             // const totalSourceQuantity = subjectBorrowQuantity;
//             // const expectedSourceTokenBalance = oldSourceTokenBalance.add(totalSourceQuantity);
//             // const newSourceTokenBalance = await dai.balanceOf(oneInchExchangeMockTowsteth.address);
//             // expect(newSourceTokenBalance).to.eq(expectedSourceTokenBalance);
//           });
...
//       subjectCaller = owner;
//       subjectTradeData = await uniswapV3ExchangeAdapterV2.generateDataParam(
//         [wsteth.address, dai.address], // Swap path
//         [500], // Send quantity
//         true,
//       );
//     };
...
//       it("should transfer the correct components to the exchange", async () => {
//         const oldSourceTokenBalance = await wsteth.balanceOf(wstethDaiPool.address);

//         await subject();
//         const totalSourceQuantity = subjectRedeemQuantity;
//         const expectedSourceTokenBalance = oldSourceTokenBalance.add(totalSourceQuantity);
//         const newSourceTokenBalance = await wsteth.balanceOf(wstethDaiPool.address);
...
//         //   dai.address,
//         // ]);
//         const oldDestinationTokenBalance = await dai.balanceOf(wstethDaiPool.address);
...
//         //   totalDestinationQuantity,
//         // );
//         const newDestinationTokenBalance = await dai.balanceOf(wstethDaiPool.address);
...
//         const swapPromise = waitForEvent(wstethDaiPool, "Swap");
//         const tx = await subject();
...
//     context("when awsteth and aDAI are collateral and wsteth and DAI are borrow assets", async () => {
//       const initializeContracts = async () => {
//         setToken = await createSetToken(
//           [awsteth.address, aDAI.address],
//           [ether(2), ether(1000)],
//           [morphoLeverageModule.address, debtIssuanceModule.address],
//         );
//         await initializeDebtIssuanceModule(setToken.address);
...
//           await morphoLeverageModule.initialize(
//             setToken.address,
//             [wsteth.address, dai.address, wbtc.address], // Enable WBTC that is not a Set position
//             [dai.address, wsteth.address, wbtc.address],
//           );
//         }
...
//         expect(JSON.stringify(collateralAssets)).to.eq(JSON.stringify([wsteth.address, dai.address]));
//         expect(isDaiCollateral).to.be.true;
//       });
...
//           dai.address,
//         );
//         expect(JSON.stringify(borrowAssets)).to.eq(JSON.stringify([wsteth.address, dai.address]));
//         expect(isDAIBorrow).to.be.true;
//       });
...
//         const issueQuantity = ether(1);
//         await debtIssuanceModule.issue(setToken.address, issueQuantity, owner.address);
//         // Lever both aDAI and awsteth in SetToken
//         if (isInitialized) {
//           const leverEthTradeData = await uniswapV3ExchangeAdapterV2.generateDataParam(
//             [dai.address, wsteth.address], // Swap path
//             [500], // fees
...
//     context("when awsteth is collateral and DAI is borrow asset", async () => {
//       before(async () => {
//         isInitialized = true;
//       });
//       cacheBeforeEach(async () => {
...
//       // Initialize module if set to true
//       if (isInitialized) {
//         await morphoLeverageModule.initialize(setToken.address, [wsteth.address, dai.address], []);
//       }
//     };
//     const initializeSubjectVariables = () => {
//       subjectSetToken = setToken.address;
