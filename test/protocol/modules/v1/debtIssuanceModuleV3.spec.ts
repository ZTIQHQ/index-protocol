...
import { ether, preciseDiv, preciseMul, preciseMulCeil } from "@utils/index";
import {
...
describe("DebtIssuanceModuleV3", () => {
  ...
  let debtIssuance: DebtIssuanceModuleV3;
  ...
  before(async () => {
    ...
    tokenWithRoundingError = await deployer.mocks.deployTokenWithErrorMock(
      owner.address,
      ether(1000000),
      ZERO,
    );
    debtIssuance = await deployer.modules.deployDebtIssuanceModuleV3(
  ...
  context("when DebtIssuanceModuleV3 is initialized", async () => {
    ...
    context("when SetToken components do not have any rounding error", async () => {
      ...
      // tokenWithRoundingError instead of weth as a default position. This is to ensure the DebtIssuanceModuleV3 behaves
      ...
      describe("#issue", async () => {
        ...
        beforeEach(async () => {
          ...
          const [, equityFlows] = await debtIssuance.getRequiredComponentIssuanceUnits(
            setToken.address,
            ether(1),
          );
          await tokenWithRoundingError.approve(
            debtIssuance.address,
            equityFlows[0].mul(ether(1.005)),
          );
        ...
        it("should have the correct token balances", async () => {
          const preMinterWethBalance = await tokenWithRoundingError.balanceOf(
          ...
          const postMinterWethBalance = await tokenWithRoundingError.balanceOf(
            subjectCaller.address,
          );
          ...
          expect(postMinterWethBalance).to.eq(
            preMinterWethBalance.sub(wethFlows).sub(tokenTransferBuffer),
          );
        ...
        it("should emit the correct SetTokenIssued event", async () => {
          ...
          await expect(subject())
            .to.emit(debtIssuance, "SetTokenIssued")
            .withArgs(
              setToken.address,
              subjectCaller.address,
              subjectTo,
        ...
        describe("when an external equity position is in place", async () => {
          ...
          before(async () => {
            await externalPositionModule.addExternalPosition(
              setToken.address,
              tokenWithRoundingError.address,
              externalUnits,
            );
          ...
          it("should have the correct token balances", async () => {
            ...
            const preSetWethBalance = await tokenWithRoundingError.balanceOf(subjectSetToken);
            const preExternalWethBalance = await tokenWithRoundingError.balanceOf(
              externalPositionModule.address,
            );
            const preMinterDaiBalance = await setup.dai.balanceOf(subjectCaller.address);
            const preSetDaiBalance = await setup.dai.balanceOf(subjectSetToken);
            ...
            const postSetWethBalance = await tokenWithRoundingError.balanceOf(subjectSetToken);
            const postExternalWethBalance = await tokenWithRoundingError.balanceOf(
              externalPositionModule.address,
            );
            const postMinterDaiBalance = await setup.dai.balanceOf(subjectCaller.address);
            const postSetDaiBalance = await setup.dai.balanceOf(subjectSetToken);
            ...
            expect(postMinterWethBalance).to.eq(
              preMinterWethBalance
                .sub(wethDefaultFlows.add(wethExternalFlows))
                .sub(tokenTransferBuffer),
            );
            expect(postSetWethBalance).to.eq(
        ...
        describe("when the manager issuance fee is 0", async () => {
          ...
          it("should have the correct token balances", async () => {
            ...
            expect(postMinterWethBalance).to.eq(
              preMinterWethBalance.sub(wethDefaultFlows).sub(tokenTransferBuffer),
            );
            expect(postSetWethBalance).to.eq(
              preSetWethBalance.add(wethDefaultFlows).add(tokenTransferBuffer),
            );
        ...
        describe("when protocol fees are enabled", async () => {
          const protocolFee: BigNumber = ether(0.2);
      ...
      describe("#redeem", async () => {
        ...
        beforeEach(async () => {
          ...
          const [, equityFlows] = await debtIssuance.getRequiredComponentRedemptionUnits(
            setToken.address,
            ether(1),
          );
          await tokenWithRoundingError.approve(
        ...
        async function subject(): Promise<ContractTransaction> {
          return debtIssuance
            .connect(subjectCaller.wallet)
            .redeem(subjectSetToken, subjectQuantity, subjectTo);
        }
        ...
        it("should have the correct token balances", async () => {
          ...
          expect(postToWethBalance).to.eq(preToWethBalance.add(wethFlows).sub(tokenTransferBuffer));
          expect(postSetWethBalance).to.eq(
        ...
        it("should emit the correct SetTokenRedeemed event", async () => {
          ...
          await expect(subject())
            .to.emit(debtIssuance, "SetTokenRedeemed")
            .withArgs(
              setToken.address,
              subjectCaller.address,
              subjectTo,
        ...
        describe("when an external equity position is in place", async () => {
          ...
          it("should have the correct token balances", async () => {
            ...
            expect(postToWethBalance).to.eq(
              preToWethBalance
                .add(wethExternalFlows.add(wethDefaultFlows))
                .sub(tokenTransferBuffer),
            );
            expect(postSetWethBalance).to.eq(
        ...
        describe("when the manager redemption fee is 0", async () => {
          ...
          it("should have the correct token balances", async () => {
            ...
            expect(postToWethBalance).to.eq(
              preToWethBalance.add(wethFlows).sub(tokenTransferBuffer),
            );
            expect(postSetWethBalance).to.eq(
              preSetWethBalance.sub(wethFlows).add(tokenTransferBuffer),
            );
      ...
      describe("#getRequiredComponentIssuanceUnits", async () => {
        ...
        async function subject(): Promise<any> {
          return debtIssuance.getRequiredComponentIssuanceUnits(subjectSetToken, subjectQuantity);
        }
    ...
    context("when SetToken components do have rounding errors", async () => {
      describe("#issue", async () => {
        ...
        describe("when rounding error is greater than tokenTransferBuffer", async () => {
          beforeEach(async () => {
            await tokenWithRoundingError.setError(-tokenTransferBuffer - 1);
          });
        ...
        describe("when rounding error is not greater than tokenTransferBuffer", async () => {
          beforeEach(async () => {
            await tokenWithRoundingError.setError(-tokenTransferBuffer);
          });
      ...
      describe("#getRequiredComponentIssuanceUnits", async () => {
        ...
        const debtUnits: BigNumber = ether(100);
        const accruedBalance = ether(0.00001);
        ...
        describe("when tokens have been issued", async () => {
          ...
          it("should return the correct issue token amounts", async () => {
            ...
            const wethFlows = preciseMulCeil(
              mintQuantity,
              preciseDiv(ether(1.005).add(accruedBalance), ether(1.005)),
            );
          ...
          describe("when an additive external equity position is in place", async () => {
            ...
            it("should return the correct issue token amounts", async () => {
              ...
              const wethFlows = preciseMulCeil(
                mintQuantity,
                preciseDiv(ether(1.005).add(accruedBalance), ether(1.005)).add(externalUnits),
              );
